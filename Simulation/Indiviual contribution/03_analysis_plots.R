...
# ------------------------------------------------------------------------------
# PART 3: Selective Uptake Comparison
# ------------------------------------------------------------------------------

comparison_data <- bind_rows(
  simulation_beta0 %>% mutate(Condition = "beta = 0"),
  simulation_beta1 %>% mutate(Condition = "beta = 1"),
  simulation_beta2 %>% mutate(Condition = "beta = 2")
)

group_shifts <- comparison_data %>%
...
p5 <- ggplot(group_shifts, aes(x = Condition, y = GroupShift)) +
  theme_bw(base_size = 14) +
  geom_boxplot(fill = "white", color = "black", width = 0.5) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3.5,
               fill = "firebrick", color = "black", stroke = 1) +
  stat_summary(fun = mean, geom = "text",
               aes(label = sprintf("%.3f", after_stat(y))),
               vjust = -1.2, color = "firebrick", fontface = "bold", size = 3.5) +
  labs(
    title = "Effect of Selective Uptake on Group Polarization",
    subtitle = "Group-level shift as a function of selective uptake strength",
    x = "Selective Uptake Parameter",
    y = "Group Polarization (Mean T_post - Mean T_pre)"
  )

ggsave("Plots/05_selective_uptake_comparison.png", plot = p5, width = 8, height = 6, dpi = 300)