...
#' Sample Additional Arguments with Selective Uptake
#'
#' Draws additional arguments from the pool with probabilities that depend on
#' the congruence between the agent's pre-discussion tendency and the argument value.
#'
#' @param pool Numeric vector. The group argument pool.
#' @param n Integer. Number of additional arguments to sample.
#' @param t_pre Numeric. Agent's pre-discussion tendency.
#' @param beta Numeric. Strength of selective uptake.
#'        beta = 0: no selectivity (baseline model)
#'        beta > 0: congruent arguments are more likely to be sampled
#'
#' @return Numeric vector of sampled arguments.

sample_selective_arguments <- function(pool, n, t_pre, beta = 0) {
  weights <- exp(beta * t_pre * pool)
  
  # Safety fallback
  if (any(!is.finite(weights)) || sum(weights) == 0) {
    weights <- rep(1, length(pool))
  }
  
  sample(pool, size = n, replace = TRUE, prob = weights)
}
...
#'
#' Simulates the discussion phase where an agent is exposed to additional arguments (AA).
...
discuss_and_update <- function(agent, pool, n_AA, C, beta = 0) {
  # Draw additional arguments (AA_i) with selective uptake
  aa_i <- sample_selective_arguments(
    pool = pool,
    n = n_AA,
    t_pre = agent$t_pre,
    beta = beta
...
#' @param beta Numeric. Strength of selective uptake. Default is 0.
#'
...
run_simulation <- function(n_agents = 10,
  ...
  for (i in 1:n_agents) {
    agents[[i]] <- discuss_and_update(agents[[i]], group_pool, n_AA, C, beta)
  }
...
#' @param ... Additional arguments passed to run_simulation (e.g., pool_bias, C, beta).
#'
